...
object ConferenceReviewing:
  ...
  private class ConferenceReviewingImpl extends ConferenceReviewing:
    ...
    private final val RELEVANCE_SCORE_THRESHOLD = 8
    private final val SCORE_NORMALIZATION_FACTOR = 10.0
    ...
    private def isOneRelevanceScoreHighEnough(article: Int): Boolean =
      scoresForArticle(article).exists { case (_, scores) =>
        scores.exists { case (question, score) => question == Question.RELEVANCE && score >= RELEVANCE_SCORE_THRESHOLD }
      }
    ...
    private def weightedFinalScore(finalScore: Int, confidence: Int): Double =
      finalScore * confidence / SCORE_NORMALIZATION_FACTOR

    override def averageWeightedFinalScore(article: Int): Double =
      val weightedFinalScores = scoresForArticle(article)
        .map((_, scores) => weightedFinalScore(scores(Question.FINAL), scores(Question.CONFIDENCE)))
